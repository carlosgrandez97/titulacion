...
if (isset($_POST['js_GuardaCliente'])) {
    ...
    $query = "INSERT INTO tbl_Cliente VALUES('".$nuevoCodigo[0]."') ;";
    $stmt = $conn->prepare($query);
    $stmt->execute();
    ...
    $query2 = "SELECT COUNT(*) FROM tbl_Cliente WHERE cPerCod = '".$nuevoCodigo[0]."'";
...
if (isset($_POST['js_ListarClientesVenta'])) {
    $inputBuscar = $_POST['inputBuscar'];
    $query = "
                SELECT p.cPerCod, CONCAT(REPLACE(p.cPerNombres, '/', ' ') , ' ', p.cPerApellidoPar , ' ', p.cPerApellidoMat ) AS Nombres
                FROM tbl_persona p
                INNER JOIN tbl_cliente c ON p.cPerCod = c.cPerCod
                WHERE 
                    p.cPerNombres LIKE '%".$inputBuscar."%'
                    OR 
                    p.cPerApellidoPar LIKE '%".$inputBuscar."%'
                    OR
                    p.cPerApellidoMat LIKE '%".$inputBuscar."% '
                    AND p.bEstado = 0
    ";
    $stmt = $conn->prepare($query);
    $stmt->execute();
